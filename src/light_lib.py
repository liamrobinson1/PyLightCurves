...
class Brdf:
    ...
    def validate(self, name: str, cd: float, cs: float, n: int):
        ...
        assert cd >= 0 and cs >= 0, ValueError("cd and cs must be >= 0 for energy conservation")
    ...
    def eval_normalized_brightness(self, L: np.array, O: np.array, N: np.array) -> np.array:
        fr = np.abs(self.eval(L, O, N))
        lc = fr * rdot(N, O) * rdot(N, L)
        return lc
    
    def compute_reflection_matrix(self, L: np.array, O: np.array, N: np.array) -> np.ndarray:
        assert L.shape[0] == O.shape[0],ValueError("Size of illumination vectors must match observer vectors")
        geom_count = L.shape[0]
        normal_count = N.shape[0]
        [v1_grid, v2_grid] = np.meshgrid(np.arange(0,geom_count), 
                                         np.arange(0,normal_count)) 
