...
class Object:
    def __init__(self, obj_path: str = None, obj_vf: Tuple[np.array, np.array] = None, is_dual: bool = False):
        if obj_path is not None:
            self._obj = pywavefront.Wavefront(obj_path, 
    ...
    def unique_areas_and_normals(self):
        (self.unique_normals, self.all_to_unique, self.unique_to_all) \
            = unique_rows(self.face_normals, return_index=True, return_inverse=True)
    ...
    def get_inner_vertices(self):
        ...
        self.verts_within_convhull = np.setdiff1d(np.arange(0,nvert), np.unique(self.f))
        
    def flip_normals(self):
        n_points_in = dot(self.face_centroids, self.face_normals).flatten() < 0
        if any(n_points_in):
            self.f[n_points_in, :] = np.array([self.f[n_points_in, 0],
                                                self.f[n_points_in, 2],
                                                self.f[n_points_in, 1]]).T
            self.build_properties()
    ...
    def render(self, render_mode: str = 'solid'):
        ...
        plt.gca().plot_trisurf(self.v[:,0], self.v[:,1], self.v[:,2], 
                triangles=self.f, 
                color=(0.7,0.7,0.7), shade=True, linewidth=linewidth)
...
def construct_from_egi_and_supports(egi: np.array, support: np.array) -> Object:
    ...
    rec_obj_verts = np.reshape(np.linalg.solve(dual_obj.fv, b), (dual_obj.f.shape[0], 3))
...
def support_reconstruction_error(egi: np.array, support: np.ndarray) -> float:
    ...
    normal_diff = vecnorm(rec_obj.unique_normals[v2_grid.flatten(),:] 
                   - hat(egi[v1_grid.flatten(),:]))
    ...
    area_error_matching = np.sum(np.abs(vecnorm(egi[matching_normals[:,0]]).flatten()
    ...
    if ref_normals_missing.size:
        ...
        all_missing_dual_vertices = rec_dual.v[ref_normals_missing[v1_grid.flatten()],:]
        all_dual_normals = rec_dual.unique_normals[v2_grid.flatten(),:]
        all_dual_supports = rec_dual.supports[v2_grid.flatten(),:]
        missing_pt_dists_to_hull = points_to_planes(pt=all_missing_dual_vertices, 
                                                    plane_n=all_dual_normals, 
                                                    support=all_dual_supports)
        missing_pt_dists_to_hull = np.reshape(missing_pt_dists_to_hull, (nmiss, ndualface), order='F')
        min_missing_dists_to_hull = np.min(np.abs(missing_pt_dists_to_hull), axis=1, keepdims=True)
        support_error = np.sum(min_missing_dists_to_hull)

    err = area_error_matching + 20*support_error + area_error_missing
    print(f"{err:.2e} | [{support_error:.2e}, {area_error_missing:.2e}, {area_error_matching:.2e}] | {ref_normals_missing.size}/{nref} missing!")
    return err
...
def optimize_supports(egi: np.array) -> Object:
    ...
    res = scipy.optimize.minimize(fun, h0, method='SLSQP',
