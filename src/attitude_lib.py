...
def rigid_rotation_dynamics(itensor: np.ndarray, w: np.ndarray, 
                            torque: np.ndarray = np.array([0,0,0])) \
                            -> np.ndarray:
    dwdt = np.zeros((3,))
...
def gravity_gradient_torque(itensor: np.ndarray, rvec: np.ndarray, 
                            mu: np.ndarray = AstroConstants.earth_mu) \
                            -> np.ndarray:
    rmag = np.linalg.norm(rvec); # Distance from central body COM to satellite COM
    return 3*mu / rmag**5 * np.cross(rvec, itensor @ rvec); # Torque applied to body

def quat_kinematics(q: np.ndarray, w: np.ndarray) -> np.ndarray:
    return 1/2 * np.array([[q[3], -q[2], q[1], q[0]],
            [q[2], q[3], -q[0], q[1]],
            [-q[1], q[0], q[3], q[2]],
            [-q[0], -q[1], -q[2], q[3]]]) \
                @ np.concatenate((w,[0]))
...
def mrp_to_quat(s: np.ndarray) -> np.ndarray:
    s2 = np.sum(s*s, axis=1, keepdims=1)
...
def axis_rotation_matrices() -> Tuple[np.ndarray]:
    r1 = lambda t: np.array([[1, 0, 0],
                            [0, np.cos(t), np.sin(t)],
                            [0, -np.sin(t), np.cos(t)]])
    
    r2 = lambda t: np.array([[np.cos(t), 0, -np.sin(t)],
                            [0, 1, 0],
                            [np.sin(t), 0, np.cos(t)]])
    
    r3 = lambda t: np.array([[np.cos(t), np.sin(t), 0],
                            [-np.sin(t), np.cos(t), 0],
                            [0, 0, 1]])
