...
fun = lambda t, y: \
    np.concatenate(
        (quat_kinematics(y[:4], y[4:]), 
         rigid_rotation_dynamics(i, y[4:], m)))
tspan = [0,10]
teval = np.linspace(tspan[0], tspan[1], int(1e3))
y0 = np.concatenate((q0,w0))
tol = 1e-13
ode_res = scipy.integrate.solve_ivp(fun, tspan, y0, 
                                    t_eval=teval,
                                    rtol=tol,
                                    atol=tol)
...
a_candidates = np.expand_dims(scipy.optimize.nnls(g_candidates, lc.flatten())[0], axis=1)
