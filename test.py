...
class MyTestMethods(unittest.TestCase):
    def assertAlmostVectorEqual(self, v1: np.array, v2: np.array, **kwargs):
        self.assertAlmostEqual(np.sum(vecnorm(v1-v2)), 0.0, **kwargs)

    def assertAlmostMatrixEqual(self, m1: np.array, m2: np.array, **kwargs):
...
class TestAttitude(MyTestMethods):
    ...
    def test_identities(self):
        self.assertAlmostMatrixEqual(quat_to_dcm(np.array([[0,0,0,1]])),
                                     np.eye(3))
        self.assertAlmostMatrixEqual(quat_to_mrp(np.array([[0,0,0,1]])),
                                     np.zeros((3,)))
        self.assertAlmostMatrixEqual(quat_to_rv(np.array([[0,0,0,1]])),
                                     np.zeros((3,)))
    ...
    def test_ang(self):
        q = rv_to_quat(v)
        self.assertAlmostVectorEqual(np.array([1.651890372817769, 
                                               1.651890372817769]),
...
class TestMathUtility(MyTestMethods):
    ...
    def test_hat(self):
        self.assertAlmostVectorEqual(hat(np.array([0,0,0])), 
                                     np.array([0,0,0]))
...
class TestOrbits(MyTestMethods):
    ...
    def test_gg_torque(self):
        ...
        m_gg_truth = np.array([0,
                               -0.700039616569510e-6,
                               -0.068416322484417e-6])
    ...
    def test_quat_kinematics(self):
        ...
        qdot_truth = np.array([-0.275436493769121,
                               0.107546296298775,
                               -0.275436493769121,
                               -0.765965580135793])
...
class TestObserver(MyTestMethods):
    ...
    def test_lla_to_itrf(self):
        itrf_true = np.array([2.587065541531238e3,
                              4.480928960442796e3,
                              3.734060588150026e3])
    ...
    def test_lla_to_eci(self):
        eci_true = np.array([4.192331656461110e3,
                              -3.032488705164016e3,
                              3.734060588150026e3])
...
class TestCoordinates(MyTestMethods):
    ...
    def test_sun(self):
        sun_true = np.array([-0.214815336337292,
                             -0.881911527862012,
                             -0.382300897809063])
...
class TestEngine(MyTestMethods):
    ...
    def test_dir_exists(self):
        ...
        self.assertTrue(os.path.exists(lce_dir), 
    ...
    def test_dir_contains_lce(self):
        ...
        self.assertTrue(os.path.exists(path_to_c), 
                        msg="LightCurveEngine.c not in provided directory!")
        self.assertTrue(os.path.exists(path_to_exec), 
